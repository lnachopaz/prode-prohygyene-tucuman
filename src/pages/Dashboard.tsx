...
import { getMultiplierInfo, formatPoints } from "@/lib/scoring";
...
export default function Dashboard() {
  ...
  const { data: nextMatches } = useQuery({
    ...
    refetchInterval: 60000, // refrescar cada 60s para captar partidos que pasan a en vivo
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {nextMatches && nextMatches[0] && (() => {
        ...
        const multInfo = getMultiplierInfo(featured.team_a, featured.team_b, featured.stage, featured.point_multiplier, featured.team_a_multiplier, featured.team_b_multiplier);
        return (
          <Card className={isLive ? "border-destructive/50 bg-gradient-to-br from-destructive/10 to-transparent" : "border-primary/30 bg-gradient-to-br from-primary/10 to-transparent"}>
            <CardContent className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="min-w-0">
                ...
                <div className="text-lg font-bold truncate">{translateTeamName(featured.team_a)} vs {translateTeamName(featured.team_b)}</div>
              ...
              <div key={m.id} className="flex items-center justify-between gap-2 py-2 border-b last:border-0">
                <div className="min-w-0">
                  <div className="text-xs text-muted-foreground truncate">
                    ...
                    {m.group_name ? ` · ${formatGroupName(m.group_name)}` : ""}
                  </div>
                  <div className="font-medium text-sm truncate">{translateTeamName(m.team_a)} vs {translateTeamName(m.team_b)}</div>
                </div>

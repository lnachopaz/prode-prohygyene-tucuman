...
import { formatPoints, sumPoints } from "@/lib/scoring";
...
type Profile = { id: string; display_name: string; avatar_url: string | null; show_in_ranking: boolean };
...
function aggregate(rows: PredRow[], profiles: Profile[], filter: StageKey): Agg[] {
  ...
  for (const [user_id, list] of byUser) {
    ...
    for (const r of finished) {
      ...
      const m = r.match!;
      const isExact = m.score_a != null && m.score_b != null && r.pred_a === m.score_a && r.pred_b === m.score_b;
      const isResult = !isExact && m.score_a != null && m.score_b != null &&
        Math.sign(r.pred_a - r.pred_b) === Math.sign(m.score_a - m.score_b);
      if (isExact) ex++;
...
export default function Ranking() {
  ...
  const visibleProfiles = useMemo(
    () => (profiles ?? []).filter((p) => p.show_in_ranking !== false),
    [profiles],
  );
  ...
  const current = useMemo<Agg[]>(() => {
    if (filter === "all") {
      ...
      return leaderboard
        .filter((r) => profMap.get(r.user_id)?.show_in_ranking !== false)
        .map((r) => ({
          user_id: r.user_id,
    ...
    if (!preds || !visibleProfiles) return [];
    return aggregate(preds, visibleProfiles, filter);
  }, [filter, leaderboard, preds, visibleProfiles, streakByUser, profMap, matchStats?.finished]);

  const previous = useMemo(() => {
    if (filter !== "all" || !preds || !visibleProfiles) return new Map<string, number>();
    ...
    const prev = aggregate(filtered, visibleProfiles, "all");
    const map = new Map<string, number>();
    prev.forEach((p, i) => map.set(p.user_id, i + 1));
    return map;
  }, [filter, preds, visibleProfiles]);
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-4">
            ...
            {(myStats.gapLeader > 0 || myStats.gapNext !== null || myStats.gapBelow !== null) && (
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-xs text-muted-foreground">
                {myStats.gapLeader > 0 && (
                  <span>−<strong className="text-foreground">{formatPoints(myStats.gapLeader)}</strong> del líder</span>
                )}
                {myStats.gapNext !== null && myStats.gapNext > 0 && (
                  <span>−<strong className="text-foreground">{formatPoints(myStats.gapNext)}</strong> del #{myStats.pos - 1}</span>
                )}
                {myStats.gapBelow !== null && myStats.gapBelow > 0 && (
                  <span>+<strong className="text-success">{formatPoints(myStats.gapBelow)}</strong> sobre #{myStats.pos + 1}</span>
                )}
              </div>
            )}
          </CardContent>
      ...
      {/* Ranking list — main table, displayed first */}
      <Card>

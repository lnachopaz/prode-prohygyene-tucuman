...
export function MatchDetailsDialog({ match }: { match: MatchDetails }) {
  ...
  const hits = data?.filter((p) => Number(p.points) >= 3).length ?? 0;
  const partial = data?.filter((p) => Number(p.points) > 0 && Number(p.points) < 3).length ?? 0;
          ...
          <div className="rounded-lg border bg-muted/30 p-4">
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
              <div className="flex items-center gap-2 justify-end min-w-0">
                <span className="font-semibold truncate">{translateTeamName(match.team_a)}</span>
                {flagA && <img src={flagA} alt={translateTeamName(match.team_a)} className="h-8 w-8 rounded-full object-cover" />}
              </div>
              <div className="text-3xl font-bold tabular-nums">
                {match.score_a ?? "-"} <span className="text-muted-foreground">:</span> {match.score_b ?? "-"}
              </div>
              <div className="flex items-center gap-2 justify-start min-w-0">
                {flagB && <img src={flagB} alt={translateTeamName(match.team_b)} className="h-8 w-8 rounded-full object-cover" />}
                <span className="font-semibold truncate">{translateTeamName(match.team_b)}</span>
              </div>
            </div>
                      ...
                      {data.map((p) => (
                        <TableRow key={p.user_id}>
                          ...
                          <TableCell className="text-right">
                            <Badge
                              variant={Number(p.points) >= 3 ? "default" : Number(p.points) > 0 ? "secondary" : "outline"}
                              className={
                                Number(p.points) >= 3

...
export default function Profile() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const total = Math.round(finished.reduce((s: number, r: any) => s + (Number(r.points) || 0), 0) * 10) / 10;
      const exact = finished.filter((r: any) => r.match.score_a != null && r.pred_a === r.match.score_a && r.pred_b === r.match.score_b).length;
      const result = finished.filter((r: any) => {
        const m = r.match;

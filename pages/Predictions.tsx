...
function MatchCard({
  ...
  const multCombo = multInfo && multInfo.parts.length > 1
    ? `${multInfo.parts.map((p) => formatMultiplier(p.mult) + " (" + p.label + ")").join(" × ")} = ${multInfo.label}`
    : null;
        ...
        <CardContent className="p-4 space-y-4">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {multInfo && (
                <Badge className="gap-1 bg-amber-500 hover:bg-amber-500 text-white" title={`Multiplicador: ${multInfo.reasons.join(" + ")}`}>
          ...
          {multCombo && (
            <div className="text-[11px] text-muted-foreground -mt-1">
              <Sparkles className="h-3 w-3 inline mr-1 text-amber-500" />{multCombo}
            </div>
          )}
          </div>
